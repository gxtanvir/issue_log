...
class _IssueUpdateScreenState extends State<IssueUpdateScreen> {
  ...
  void initState() {
    ...
    issueDetailsCtrl = TextEditingController(
      text: widget.issue['issue_details'] ?? '',
    );
    responsiblePersonCtrl = TextEditingController(
      text: widget.issue['responsible_person'] ?? '',
    );
    ...
    // Initialize dates
    issueRaiseDate =
        widget.issue['issue_raise_date'] != null
            ? DateTime.parse(widget.issue['issue_raise_date'])
            : DateTime.now();

    deadline =
        widget.issue['deadline'] != null
            ? DateTime.tryParse(widget.issue['deadline'])
            : null;

    completeDate =
        widget.issue['complete_date'] != null
  ...
  final body = {
    ...
    "responsible_person":
        responsibleParty == "MIS" ? ApiService.username ?? "" : responsiblePersonCtrl.text.trim(),
  ...
  try {
    ...
    if (res.statusCode == 200) {
      final updatedIssue = jsonDecode(res.body); // Parse updated issue from response

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Issue updated successfully")),
      );

      Navigator.of(context).pop(updatedIssue); // Pass updated issue back
    } else {
